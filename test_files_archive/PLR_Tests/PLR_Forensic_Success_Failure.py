# ==============================================================================
# PATH OF LEAST RESISTANCE (PLR) - TEST 39: Success vs. Failure Forensic Showdown
#
# This script analyzes 10 confirmed SUCCESSES and 10 confirmed FAILURES
# from the v16.0 champion to find the single analytic difference.
#
# GOAL: Compare the structural metrics (v_mod6, gap) of the winning
# candidates in both scenarios to identify the final f(p_n) trigger.
# ==============================================================================
...
def get_v16_final_decision(p_n, candidates_ranked_list):
    ...
    decision_reason = "Defaulted to v11.0 Winner"
    ...
    if winner_v11_vmod6 < CLEAN_THRESHOLD: # If #1 is "Clean"
        for rank_index in range(1, MAX_SIGNATURE_SEARCH_DEPTH): # Ranks 2, 3, 4
            ...
            if suspect_vmod6 > MESSY_THRESHOLD:
                ...
                decision_reason = f"TRIGGERED RANK {rank_index+1} OVERRIDE"
                break
...
START_INDEX = 10 
CASES_TO_FIND = 10 # 10 successes and 10 failures
...
def run_forensic_showdown():
    ...
    print(f"\nStarting PLR 'Forensic Showdown' (Test 39)...")
    print(f"  - Engine: v16.0 (75.94% Champion)")
    print(f"  - Searching for first {CASES_TO_FIND} SUCCESSES and {CASES_TO_FIND} FAILURES...")
    print("-" * 80)
    start_time = time.time()
    ...
    for i in range(START_INDEX, loop_end_index):
        if successes_found >= CASES_TO_FIND and failures_found >= CASES_TO_FIND:
            break 
        ...
        if is_success and successes_found < CASES_TO_FIND:
            case_type = "SUCCESS"
            case_num = successes_found + 1
            successes_found += 1
        elif not is_success and failures_found < CASES_TO_FIND:
            case_type = "FAILURE"
            case_num = failures_found + 1
            failures_found += 1
        else:
            continue

        # Print the Case File
        print(f"\n" + "="*15 + f" CASE FILE #{case_num} - {case_type} " + "="*15)
        print(f"PRIME p_n: {p_n}")
        print(f"TRUE NEXT PRIME: {true_p_n_plus_1}")
        print(f"v16.0 PREDICTION: {prediction_v16}")
        print(f"v16.0 DECISION: {decision_reason}")
        print("-" * 60)
        
        print(f"{'Rank':<5} | {'Prime (q_i)':<12} | {'v11 Score':<12} | {'v_mod6 (%)':<12} | {'Gap (g_n)':<10} | {'STATUS':<15}")
        print("-" * 75)
        ...
        for rank in range(len(candidate_scores)):
            ...
            print(f"{rank+1:<5} | {prime:<12} | {score:<12.2f} | {vmod6:<12.2f} | {gap:<10} | {status:<15}")
        
        print("="*60)
            
    # --- Final Summary ---
    print(f"\nAnalysis completed in {time.time() - start_time:.2f} seconds.")
    print(f"Found {CASES_TO_FIND} success cases and {CASES_TO_FIND} failure cases.")

if __name__ == "__main__":
    run_forensic_showdown()